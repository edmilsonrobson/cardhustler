...
public class CardVisual : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    private bool standaloneHovering;
    ...
    private void FollowRotation()
    {
        ...
        Vector3 movementRotation =
            (parentCard.isDragging ? movementDelta : movement) * rotationAmount;
    ...
    private void StandaloneTilt()
    {
        float sine = Mathf.Sin(Time.time) * (standaloneHovering ? .2f : 1);
        float cosine = Mathf.Cos(Time.time) * (standaloneHovering ? .2f : 1);

        Vector3 offset = transform.position - Camera.main.ScreenToWorldPoint(Input.mousePosition);
        float tiltX = standaloneHovering ? ((offset.y * -1) * manualTiltAmount) : 0;
        float tiltY = standaloneHovering ? ((offset.x) * manualTiltAmount) : 0;
    ...
    private void Select(Card card, bool state)
    {
        ...
        shakeParent
            .DOPunchRotation(Vector3.forward * (hoverPunchAngle / 2), hoverTransition, 20, 1)
            .SetId(2);
    ...
    private void PointerUp(Card card, bool longPress)
    {
        if (scaleAnimations)
            transform
                .DOScale(longPress ? scaleOnHover : scaleOnSelect, scaleTransition)
                .SetEase(scaleEase);
        canvas.overrideSorting = false;
    ...
    {
        if (parentCard != null && !standaloneMode)
            return;
        standaloneHovering = true;
    }
